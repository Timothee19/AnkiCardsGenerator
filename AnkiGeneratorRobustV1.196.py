    ...
    for attempt in range(retries):
        ...
        try:
            ...
            if "JSON tronqué" in str(e) and len(full_content_ref) >= 55000:
                print("   => Le JSON retourné a été tronqué à plus de 55 000 caractères. Augmentation de max_tokens à 32000 pour les prochaines tentatives.")
...
def generate_bridge_cards(client, chunks, all_cards, filename_tag="Course"):
    ...
    for chunk in chunks:
        for line in chunk.split("\n"):
            line = line.strip()
            if line.startswith("#") or (line and not line.startswith("![") and len(line) > 10):
    ...
    pairs = [(section_headers[i], section_headers[i+1]) for i in range(len(section_headers)-1)]
...
"""
    ...
    def _generate_single_bridge(secA, secB):
        ...
        for attempt in range(retries):
            if attempt == 2 and current_model == "mistral-small-latest":
                print(f"      ⚠️ Mistral Small indisponible pour la carte Bridge ({secA[:30]}). Bascule de secours (failover) sur Mistral Large...")
                current_model = "mistral-large-latest"
            try:
                response = client.chat.complete(
                    model=current_model,
                    ...
                    messages=[
                        ...
                        {"role": "user", "content": f"Section A: {secA}\nSection B: {secB}\nFilename tag: {filename_tag}"}
                    ],
                    max_tokens=8000
                )
                content = response.choices[0].message.content
                ...
                print(f"      [TENTATIVE BRIDGE {attempt+1}/{retries}] Erreur ({secA[:30]}): {e}")
                time.sleep(2)
        return None

    bridge_cards = []
    with ThreadPoolExecutor(max_workers=8) as executor:
        futures = [executor.submit(_generate_single_bridge, secA, secB) for secA, secB in pairs]
        for future in as_completed(futures):
            res = future.result()
            if res:
...
def add_card_to_decks(deck_definitions, deck_theoremes, deck_vocabulaire, deck_synthese, card, sequence=""):
...
def process_course():
    print("\n" + "="*50)
    print("GÉNÉRATEUR DE CARTES ANKI V1.19 — FAST & ROBUST DESIGN (HTML/MathJax + Clozes Natifs)")
    print("="*50 + "\n")
    ...
    try:
        ...
        for i, card in enumerate(all_cards):
            add_card_to_decks(deck_definitions, deck_theoremes, deck_vocabulaire, deck_synthese, card, sequence=f"{i:04d}")
            
        output_filename = f"{filename_without_ext}_Infaillible.apkg"
        my_package = genanki.Package([deck_definitions, deck_theoremes, deck_vocabulaire, deck_synthese])
