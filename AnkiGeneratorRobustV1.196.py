...
def safe_print(*args, **kwargs):
    with PRINT_LOCK:
        try:
            ...
            safe_args = [
                (
                    arg.encode(sys.stdout.encoding or "ascii", errors="replace").decode(
                ...
                if next_ch in LATEX_KEYWORDS:
                    for keyword in LATEX_KEYWORDS[next_ch]:
                        if raw_content.startswith(keyword, i + 2):
                            ...
                            if (
                                next_idx >= len(raw_content)
                                or not raw_content[next_idx].isalpha()
                            ):
...
def resolve_img_filename(img_num):
    # Check if there is a file in the current directory matching img-img_num.*
    try:
        import os

        for f in os.listdir("."):
            if f.lower().startswith(f"img-{img_num}.") and f.lower().endswith(
...
def parse_and_repair_json(client, raw_content, model="mistral-small-latest"):
    ...
    try:
        ...
        if repaired_data is not None:
            ...
            if not (
                normalized_end.endswith("}")
                or normalized_end.endswith("]")
                or normalized_end.endswith("}```")
                or normalized_end.endswith("]```")
            ):
                print(
                    "      [Parsing JSON] ⚠️ DANGER: Le JSON généré par l'IA est TRONQUÉ (incomplet)."
                )
                print(
                    "      [Parsing JSON] json_repair a forcé la fermeture. Rejet volontaire pour régénération complète."
                )
                raise json.JSONDecodeError(
                    "JSON tronqué détecté", clean_content, len(clean_content)
                )
        ...
        if isinstance(repair_err, json.JSONDecodeError) and "JSON tronqué" in str(
            repair_err
        ):
            raise repair_err
    ...
    print(
        "      [Parsing JSON] Parsing direct et réparation sur secured_content échoués. Tentative de secours..."
    )
    try:
        repaired_data = json_repair.loads(secured_content)
        ...
        if not (clean_content.endswith("}") or clean_content.endswith("]")):
            ...
            print(
                "      [Parsing JSON] json_repair a forcé la fermeture, limitant le résultat à 1 carte."
            )
            print(
                "      [Parsing JSON] Rejet volontaire pour forcer l'IA à relancer le chunk complet."
            )
            raise json.JSONDecodeError(
                "JSON tronqué détecté", clean_content, len(clean_content)
            )
        ...
        if repaired_data is not None:
            print(
                "      [Parsing JSON] Réparation réussie via secours sur secured_content !"
            )
            return repaired_data
        else:
            raise json.JSONDecodeError(
                "json_repair a retourné None", secured_content, len(secured_content)
            )
    except Exception as repair_err:
        ...
        try:
            ...
            response = client.chat.complete(
                ...
                messages=[
                    ...
                    {
                        "role": "user",
                        "content": f"Here is the numbered text as input, divide it strictly following the rules:\n\n{numbered_text}",
                    },
                ],
            )
...
def split_text_in_half(text):
    ...
    for offset in range(min(15, len(lines) // 2)):
        ...
        if idx2 < len(lines) - 1 and (
            not lines[idx2].strip() or lines[idx2].startswith("#")
        ):
            best_split = idx2
            break
...
"""
    ...
    if len(chunk_text) > 2000:
        ...
        print(
            "      [REPLI DIVIDE & CONQUER] Subdivision automatique du bloc en 2 sous-blocs plus simples..."
        )

        part1, part2 = split_text_in_half(chunk_text)

        print(
            f"      [REPLI DIVIDE & CONQUER] -> Lancement du sous-bloc A (taille: {len(part1)} chars)..."
        )
        cards1 = extract_cards_from_chunk(
            client, part1, filename_tag=filename_tag, model=model, retries=retries
        )

        print(
            f"      [REPLI DIVIDE & CONQUER] -> Lancement du sous-bloc B (taille: {len(part2)} chars)..."
        )
        cards2 = extract_cards_from_chunk(
            client, part2, filename_tag=filename_tag, model=model, retries=retries
        )

        if cards1 is None:
            print(
                "      [REPLI DIVIDE & CONQUER] Sous-bloc A a échoué. Mise en attente pour un rattrapage à la fin du découpage..."
            )
        if cards2 is None:
            print(
                "      [REPLI DIVIDE & CONQUER] Sous-bloc B a échoué. Mise en attente pour un rattrapage à la fin du découpage..."
            )
        ...
        if combined_cards:
            print(
                f"      [REPLI DIVIDE & CONQUER] ✅ Récupération réussie de {len(combined_cards)} cartes au total pour ce bloc !"
            )
...
E. PROACTIVE IMAGE INJECTION FORBIDDEN: NEVER add an image on your own to an "autonomous" card.
...
J. INCOMPLETE FRONT (REJECT OR REWRITE): If the front ends with ":" or "For example:" without the announced content present, complete it or fix it.
...
M. CLOZE OVERLOAD CHECK (REJECT): If a "Texte à trous" card contains more than 3 clozes (e.g. `{{c4::...}}`), REJECT IT.
...
2. If an image needs to be inserted, a reference fixed, formatting repaired (remove \text{}, add \( \)), or a missing Explanatory Context added, return "action": "rewrite", and provide the perfectly formatted text.
...
"""
    ...
    base_user_msg = (
        f"[ORIGINAL CHUNK TEXT FOR IMAGE REFERENCE AND CONTEXT]\n{chunk_text}\n\n"
        + (
            f"[IMAGE INDEX AND THEIR DESCRIPTIONS]\n"
            + "\n".join(
                [f"- {k}: {v}" for k, v in (image_descriptions or {}).items() if v]
            )
            + "\n\n"
    ...
    for attempt in range(retries):
        if attempt == 2 and current_model == "mistral-small-latest":
            print(
                "   ⚠️ Mistral Small indisponible (2 échecs). Bascule de secours (failover) sur Mistral Large..."
            )
        ...
        try:
            ...
            if "Prefix does not match the response format" in str(
                e
            ) or "invalid_request_invalid_args" in str(e):
                print(
                    f"      [TRONCATURE QA] Le préfixe a été rejeté par l'API (Erreur 400). Régénération complète sans préfixe (Tentative {attempt+1}/{retries})..."
                )
                current_messages = [
                    {"role": "system", "content": system_prompt},
                    {"role": "user", "content": base_user_msg},
            ...
            print(
                f"   Erreur QA Mistral ({current_model}) (Tentative {attempt+1}/{retries}): {e}"
            )
            full_content_ref = locals().get("full_content", locals().get("content", ""))
            if "JSON tronqué" in str(e) and len(full_content_ref) >= 55000:
                print(
                    "   => Le JSON retourné a été tronqué à plus de 55 000 caractères. Augmentation de max_tokens à 32000 pour les prochaines tentatives."
                )
                dynamic_max_tokens = 32000

            if isinstance(e, json.JSONDecodeError) and "JSON tronqué détecté" in str(e):
                print(
                    f"      [TRONCATURE QA] JSON tronqué détecté. Demande de réécriture complète à l'IA (Tentative {attempt+1}/{retries})..."
                )
                ...
                current_messages = [
                    ...
                    {
                        "role": "user",
                        "content": "Your previous JSON response was truncated. Please rewrite it ENTIRELY from the beginning, starting exactly with the JSON you already wrote above, and then complete the object. Output ONLY the valid completed JSON.",
                    },
                ]
            time.sleep(2)
...
def repair_truncated_card(
    client, card, chunk_text, reason, model="mistral-large-latest"
):
    """
    ...
    try:
        response = client.chat.complete(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "You are a professional Anki card editor. Repair truncated flashcards perfectly.",
                },
        ...
        if "front" in data and "back" in data:
            ...
            print(
                f"      [RÉPARATION RÉUSSIE] Carte réparée avec succès ! (Raison : {reason})"
            )
            return repaired
    except Exception as e:
...
def filter_truncated_cards(client, cards, chunk_text=""):
    ...
    for c in cards:
        ...
        if is_truncated:
            if client and chunk_text:
                print(
                    f"      [TRONCATURE DÉTECTÉE] Tentative de réparation de la carte : {front_clean[:80]}..."
                )
                ...
                if repaired_card:
                    ...
                    try:
                        with open("pipeline_logs.md", "a", encoding="utf-8") as f:
                            f.write(
                                "## [FILTRE TRONCATURE] Carte Réparée avec Succès\n\n"
                            )
    ...
    if rejected_count > 0 or repaired_count > 0:
        print(
            f"   => Filtre troncature : {repaired_count} carte(s) réparée(s), {rejected_count} rejetée(s), {len(filtered)} conservée(s)."
        )
    return filtered
...
def filter_cloze_overload(cards):
    ...
    for c in cards:
        ...
        if len(cloze_indices) > 3:
            ...
            print(
                f"      [FILTRE CLOZE V1.1] Carte marquée comme surchargée ({len(cloze_indices)} clozes > 3 max) : {front[:80]}..."
            )
            try:
                with open("pipeline_logs.md", "a", encoding="utf-8") as f:
    ...
    if rejected_count > 0:
        print(
            f"   => Filtre cloze overload V1.1 : {rejected_count} carte(s) rejetée(s), {len(filtered)} conservée(s)."
        )
    return filtered
...
def audit_elaboration_quota(cards):
    ...
    for c in cards:
        ...
        is_true = (not is_false) and any(
            re.search(p, front_clean) for p in true_elab_patterns
        )
    ...
    print(
        f"   => Audit Elaboration V1.1 : {elaborative_count}/{total_basique} Basique elaboratives ({quota_pct:.1f}%) -- {status}"
    )
...
def coverage_audit_agent(
    ...
    print(
        f"   => {len(under_covered)} bloc(s) sous-couverts. Generation complementaire..."
    )

    def _process_under_covered(ci, chunk, count, header):
        print(f"      -> Bloc {ci+1} ({header[:60]}...): {count} carte(s), relance...")
        try:
            supplementary = extract_cards_from_chunk(
                client, chunk, filename_tag=filename_tag
            )
            if supplementary:
                supplementary = ai_quality_control_cards(
                    client,
                    supplementary,
                    chunk_text=chunk,
    ...
    with ThreadPoolExecutor(max_workers=8) as executor:
        futures = [
            executor.submit(_process_under_covered, ci, chunk, count, header)
            for ci, chunk, count, header in under_covered
        ]
    ...
    try:
        with open("pipeline_logs.md", "a", encoding="utf-8") as f:
            ...
            f.write(
                f"- **Sous-couverts:** {len(under_covered)}\n- **Complementaires:** {len(new_cards)}\n\n---\n"
            )
    except Exception as e:
        print(f"Log Error: {e}")
...
def generate_bridge_cards(client, chunks, all_cards, filename_tag="Course"):
    ...
    for chunk in chunks:
        for line in chunk.split("\n"):
            ...
            if line.startswith("#") or (
                line and not line.startswith("![") and len(line) > 10
            ):
                section_headers.append(line.lstrip("#").strip()[:150])
                break
    ...
    pairs = [
        (section_headers[i], section_headers[i + 1])
        for i in range(len(section_headers) - 1)
    ]

    bridge_prompt = r"""
ROLE: Bridge Card Generator.
Generate EXACTLY ONE Bridge Card connecting two consecutive course sections.

RULES:
1. type: "Basique", subdeck: "Par Cœur"
2. tags: a SPACE-SEPARATED STRING (not a list). Include "Catégorie::Bridge" after the filename tag.
3. front: comparative, relational, or synthesizing question
4. back: synthesized explanation + a Contexte Explicatif section (2-3 sentences of plain-language intuition)
5. JSON: double-escape ALL backslashes (e.g. `\\frac`, `\\[`, `\\(`)
6. Format: Plain text / HTML with MathJax `\( ... \)` or `\[ ... \]`. NO `\text{}` or `\begin{aligned}`.
7. LANGUAGE: MUST be the same language as the section titles. If titles are in English, ALL card text must be in English.

OUTPUT JSON:
{"cards": [{"type": "Basique", "subdeck": "Par Cœur", "front": "...", "back": "...", "tags": "FilenameTag Catégorie::Bridge"}]}
"""

    def _generate_single_bridge(secA, secB):
        retries = 3
        current_model = "mistral-small-latest"
        for attempt in range(retries):
            if attempt == 2 and current_model == "mistral-small-latest":
                print(
                    f"      ⚠️ Mistral Small indisponible pour la carte Bridge ({secA[:30]}). Bascule de secours (failover) sur Mistral Large..."
                )
                current_model = "mistral-large-latest"
            try:
                response = client.chat.complete(
                ...
                print(
                    f"      [TENTATIVE BRIDGE {attempt+1}/{retries}] Erreur ({secA[:30]}): {e}"
                )
                time.sleep(2)
        return None
    ...
    with ThreadPoolExecutor(max_workers=8) as executor:
        futures = [
            executor.submit(_generate_single_bridge, secA, secB) for secA, secB in pairs
        ]
        for future in as_completed(futures):
            res = future.result()
            if res:
    ...
    print(
        f"   => Bridge Cards V1.1 : {len(bridge_cards)} cartes-pont pour {len(pairs)} transitions."
    )
...
def audit_image_coverage(cards, chunk_text, image_descriptions=None):
    ...
    if chunk_images:
        ...
        if missing_images:
            ...
            for missing_img in missing_images:
                ...
                if img_desc:
                    desc_words = set(
                        w.lower() for w in re.findall(r"[a-zA-Z]{3,}", img_desc)
                    )
                ...
                if best_card_idx >= 0 and best_score >= 2:
                    ...
                    injected_count += 1
                    print(
                        f"      [AUDIT IMAGE] Image manquante '{missing_img}' injectee dans carte {best_card_idx} (score: {best_score})"
                    )
                    try:
                        with open("pipeline_logs.md", "a", encoding="utf-8") as f:
                            f.write(
                                f"## [AUDIT IMAGE V1.1] Image manquante injectee\n\n"
                            )
                            f.write(f"- **Image:** {missing_img}\n")
                            f.write(f"- **Description:** {img_desc}\n")
                            f.write(
                                f"- **Carte cible (ID {best_card_idx}):** {c.get('front', '')[:80]}...\n"
                            )
                            f.write(
                                f"- **Score de correspondance:** {best_score}\n\n---\n"
                            )
                    except Exception as e:
                        print(f"Log Error: {e}")
                else:
                    warned_count += 1
                    print(
                        f"      [AUDIT IMAGE] Image '{missing_img}' non couverte (aucune carte pertinente trouvee)"
                    )
                    try:
                        with open("pipeline_logs.md", "a", encoding="utf-8") as f:
                            f.write(
                                f"## [AUDIT IMAGE V1.1] Image non couverte (avertissement)\n\n"
                            )
                            f.write(f"- **Image:** {missing_img}\n")
                            f.write(
                                f"- **Description:** {img_desc if img_desc else '(aucune annotation)'}\n"
                            )
                            f.write(
                                f"- **Raison:** Aucune carte avec un score de correspondance suffisant (meilleur: {best_score})\n\n---\n"
                            )
                    except Exception as e:
                        print(f"Log Error: {e}")
            ...
            if injected_count > 0 or warned_count > 0:
                print(
                    f"   => Audit images : {injected_count} image(s) injectee(s), {warned_count} avertissement(s)."
                )
    ...
    for c in cards:
        ...
        if re.search(mention_pattern, front) and not re.search(draw_pattern, front):
            ...
            back_imgs = set(
                ...
                + re.findall(
                    r'<img[^>]+src=["\']([^"\']+\.(?:jpeg|jpg|png|gif|webp|svg))["\']',
                    back,
                    re.IGNORECASE,
                )
            )
            ...
            for img in missing_in_front:
                ...
                print(
                    f"      [AUDIT IMAGE] Image '{img}' copiée vers le front (mention textuelle détectée)"
                )
                try:
                    with open("pipeline_logs.md", "a", encoding="utf-8") as f:
                        f.write(f"## [AUDIT IMAGE V1.13] Image poussée vers le front\n")
                        f.write(
                            f"- L'image **{img}** a été ajoutée au front car ce dernier mentionne une figure.\n"
                        )
                        f.write(
                            f"- **Nouveau Front:** {c.get('front', '')[:80]}...\n\n---\n"
                        )
                except:
                    pass
    ...
    if front_pushed_count > 0:
        print(
            f"   => Audit images : {front_pushed_count} image(s) copiée(s) vers le front."
        )
...
def supervisor_deduplicate_cards(
    client, cards, image_descriptions=None, model="mistral-large-latest", retries=5
):
    if len(cards) <= 1:
        return cards
    ...
    try:
        ...
        print(
            f"   Erreur embeddings dans le Superviseur: {e}. On continue sans déduplication."
        )
        return cards
    ...
    print(
        f"   (Agent Superviseur - Étape 1.5 : Vérification sémantique profonde sur {len(duplicate_groups)} groupes...)"
    )
    ...
    try:
        ...
        for group in duplicate_groups:
            ...
            for i in range(1, len(group)):
                ...
                if other_emb is not None:
                    ...
                    if sim >= full_similarity_threshold:
                        ...
                        print(
                            f"      [INFO] Carte {other_idx} écartée du groupe {master_idx} (sim content: {sim:.3f} < {full_similarity_threshold})"
                        )
            ...
            if len(valid_members) > 1:
                ...
                print(
                    f"      [INFO] Groupe {master_idx} dissous après vérification profonde."
                )
        ...
        print(
            f"   [ERREUR] Échec vérification profonde: {e}. On continue avec les groupes originaux par précaution."
        )

    all_deleted_ids = set()
    print(
        f"   => {len(duplicate_groups)} groupes de doublons confirmés. Lancement de la fusion ciblée..."
    )
            ...
            try:
                response = client.chat.complete(
                    ...
                    messages=[
                        ...
                        {
                            "role": "user",
                            "content": f"Cards to merge:{image_ctx}\n\n{group_json}",
                        },
                    ],
                    max_tokens=32000,
                )
                ...
                if final_cards:
                    ...
                    with fusion_lock:
                        ...
                        deleted_in_group = [
                            gid for gid in group if gid in all_deleted_ids
                        ]
...
def add_card_to_decks(
    deck_definitions, deck_theoremes, deck_vocabulaire, deck_synthese, card, sequence=""
):
    ...
    if "Bridge" in tags_str or "Elaborative_Interrogation" in tags_str:
        deck = deck_synthese
    elif is_cloze:
        deck = deck_vocabulaire
    elif subdeck_choice == "À Refaire":
        deck = deck_theoremes
    else:
        a_refaire_keywords = [
            "théorème",
        ...
        if any(kw in raw_front.lower() for kw in a_refaire_keywords):
            deck = deck_theoremes
        else:
    ...
    def auto_wrap_math(text):
        ...
        nested_braces = r"\{[^{}]*(?:\{[^{}]*\}[^{}]*)*\}"
        pattern = (
            r"([a-zA-Z0-9\x5c]+(?:\^" + nested_braces + r"|_" + nested_braces + r"))"
        )
    ...
    def auto_wrap_latex_commands(text):
        ...
        for i in range(0, len(parts), 2):
            if not parts[i].strip():
                continue
            parts[i] = re.sub(
                latex_term_pattern, lambda m: f"\\({m.group(0)}\\)", parts[i]
            )
        return "".join(parts)
    ...
    front_html = re.sub(
        robust_img_pattern, r'<img src="\2">', raw_front, flags=re.IGNORECASE
    )
    back_html = re.sub(
        robust_img_pattern, r'<img src="\2">', raw_back, flags=re.IGNORECASE
    )
    ...
    if is_cloze:
        ...
        my_note = genanki.Note(
            model=model_cloze, fields=[front_html, back_html, sequence], tags=tags
        )
        ...
        if is_generalite:
            my_note = genanki.Note(
                ...
                fields=[front_html, back_html, sequence],
                tags=tags,
            )
        else:
            my_note = genanki.Note(
                model=model_basic, fields=[front_html, back_html, sequence], tags=tags
            )
        deck.add_note(my_note)
...
def process_course():
    print("\n" + "=" * 50)
    print(
        "GÉNÉRATEUR DE CARTES ANKI V1.19 — FAST & ROBUST DESIGN (HTML/MathJax + Clozes Natifs)"
    )
    print("=" * 50 + "\n")
    ...
    print("\n" + "=" * 50)
    print("--- ANKI CARD GENERATOR FAST & ROBUST V1.19 ---")
    ...
    try:
        ...
        if len(full_markdown) < 2000:
            print(
                "   => Cours très court (< 2000 caractères), on ignore le découpage sémantique."
            )
        ...
        def process_single_chunk(idx, chunk):
            ...
            cards = audit_image_coverage(
                cards, chunk, image_descriptions=image_descriptions
            )
        ...
        print(
            f"\n5.5) Déduplication lexicale (Superviseur) : {len(all_cards)} cartes en revue..."
        )
        ...
        print(
            f"\n5.7) Audit de couverture V1.1 : vérification des sections sous-représentées..."
        )
        all_cards = coverage_audit_agent(
            client,
            all_cards,
            chunks,
            full_markdown,
            filename_tag=filename_without_ext,
            image_descriptions=image_descriptions,
        )

        print(f"\n5.8) Génération de cartes-pont inter-sections V1.1...")
        bridge_cards = generate_bridge_cards(
            client, chunks, all_cards, filename_tag=filename_without_ext
        )
        if bridge_cards:
            all_cards.extend(bridge_cards)

        print(
            f"\n5.9) Assemblage final V1.1 : {len(all_cards)} cartes, compilation du paquet Anki..."
        )
        ...
        deck_theoremes = genanki.Deck(
            deck_id_2, f"{deck_name}::02_Théorèmes_et_Preuves"
        )
        deck_vocabulaire = genanki.Deck(
            deck_id_3, f"{deck_name}::03_Vocabulaire_et_Constantes"
        )
        deck_synthese = genanki.Deck(
            deck_id_4, f"{deck_name}::04_Synthèse_et_Relations"
        )
        ...
        my_package = genanki.Package(
            [deck_definitions, deck_theoremes, deck_vocabulaire, deck_synthese]
        )
