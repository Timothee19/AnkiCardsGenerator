    ...
    for attempt in range(retries):
        ...
        try:
            ...
            if "JSON tronqué" in str(e) and len(full_content_ref) >= 50000:
                print(
                    "   => Le JSON retourné a été tronqué à plus de 50 000 caractères. Augmentation de max_tokens à 32000 pour les prochaines tentatives."
                )
...
"""
    ...
    def _generate_single_bridge(idx, secA, secB):
        ...
        for attempt in range(retries):
            if attempt == 2 and current_model == "mistral-small-latest":
                print(
                    f"      ⚠️ Mistral Small indisponible pour la carte Bridge. Bascule de secours (failover) sur Mistral Large..."
                )
                current_model = "mistral-large-latest"
            try:
                response = client.chat.complete(
                ...
                if data.get("is_bridge_useful") and "cards" in data and data["cards"]:
                    ...
                    bc["tags"] = tags_val
                    print(
                        f"      [BRIDGE OK] Lien utile trouvé entre bloc {idx+1} et {idx+2}."
                    )
                    return bc
                else:
                    print(
                        f"      [BRIDGE REJETÉ] Aucun lien pertinent entre bloc {idx+1} et {idx+2}."
                    )
                    return None
            except Exception as e:
    ...
    with ThreadPoolExecutor(max_workers=8) as executor:
        futures = [
            executor.submit(_generate_single_bridge, i, secA, secB)
            for i, (secA, secB) in enumerate(pairs)
        ]
        for future in as_completed(futures):
            res = future.result()
            if res:
...
def add_card_to_decks(
    ...
    if (
        "Bridge" in tags_str
        or "Elaborative_Interrogation" in tags_str
        or subdeck_choice == "04_Synthèse_et_Relations"
    ):
        deck = deck_synthese
...
def process_course():
    print("\n" + "=" * 50)
    print(
        "GÉNÉRATEUR DE CARTES ANKI V1.3 — FAST & ROBUST DESIGN (HTML/MathJax + Clozes Natifs)"
    )
    print("=" * 50 + "\n")
    ...
    try:
        ...
        deck_exercices = genanki.Deck(
            deck_id_5, f"{deck_name}::05_Exercices_et_Exemples"
        )
