    ...
    },
    {
        ...
        "function": {
            ...
            "parameters": {
                ...
                "required": ["sub_nodes", "parent_node"],
                "properties": {
                    "sub_nodes": {
                        "type": "array",
                        "items": {
...
def agent_builder_loop(markdown_lines_str, tools, sys_prompt, client):
    ...
    input = [
        ...
        {"role": "user", "content": markdown_lines_str},
    ]
    # 2. La boucle infinie agentique
    while True:
        response = client.chat.complete(
            model="mistral-large-latest", messages=input, tools=tools, temperature=0.0
        )
        ...
        if not assistant_message.tool_calls:
            print(
                "✅ L'agent a terminé son travail de construction et de vérification !"
            )
            break
        ...
        for tool_call in assistant_message.tool_calls:
            ...
            if nom_fonction == "add_main_node":
                ...
                for node_data in nodes:
                    ...
                    G.add_node(
                        main_id, label=label, is_main=True, first_line=first_line
                    )
                    covered_lines.add(
                        first_line
                    )  # On déclare cette ligne comme couverte
                ...
                if not G.has_node(parent_node):
                    resultat_outil = (
                        f"Erreur : Le parent_node '{parent_node}' n'existe pas."
                    )
                else:
                ...
                if matches:
                    ...
                    resultat_outil = (
                        f"Résultat(s) trouvé(s) pour '{blind_ref}' :\n"
                        + "\n".join(matches[:10])
                    )
                    if len(matches) > 10:
                        resultat_outil += (
                    ...
                    resultat_outil = (
                        f"Aucun résultat trouvé pour la référence '{blind_ref}'."
                    )

            elif nom_fonction == "read_graph_main_nodes":
                main_nodes_info = [
                    f"{n} (Ligne {d.get('first_line', '?')})"
                ...
                if not missing_lines:
                    resultat_outil = (
                        "Exhaustivity Check success : 100% du document est couvert."
                    )
                    print(
                        "✅ Intégralité respectée : 100% du Markdown a été retranscrit !"
                    )
                else:
                    taux = (
                        (total_lines_count - len(missing_lines)) / total_lines_count
                    ) * 100
                    print(
                        f"⚠️ Retranscription incomplète : {taux:.1f}% des lignes couvertes."
                    )
                    ...
                    with open(report_file, "w", encoding="utf-8") as f:
                        f.write(
                            f"=== RAPPORT DES {len(missing_lines)} LIGNES MANQUANTES ===\n\n"
                        )
                    ...
                    print(
                        f"📁 Le détail complet a été sauvegardé dans le fichier : {report_file}"
                    )
                    ...
                    main_nodes_list = [
                        n for n, d in G.nodes(data=True) if d.get("is_main")
                    ]
...
my_deck = genanki.Deck(generer_id_deterministe("Lebesgue"), "Lebesgue")
