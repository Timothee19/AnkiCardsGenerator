...
def semantic_split_with_ai(
    client, markdown_text, learning_depth, model="mistral-large-latest", retries=2
):
    lines = markdown_text.split("\n")
...
def remove_code_tags_around_mathjax(text):
    ...
    text = re.sub(
        r"<code[^>]*>\s*(\\(.*?\\)|\\[.*?\\])\s*</code>", r"\1", text, flags=re.DOTALL
    )
    return text
    ...
    def _process_under_covered(ci, chunk, count, header):
        ...
        try:
            supplementary = extract_cards_from_chunk(
                client, chunk, "Zero Lecture", filename_tag=filename_tag
            )
            if supplementary:
...
def supervisor_deduplicate_cards(
    ...
    try:
        for i in range(0, len(texts), batch_size):
            ...
            report_progress(
                3,
                f"Calcul des embeddings... ({len(all_embeddings)}/{len(texts)})",
                ratio,
            )
    ...
    try:
        ...
        for i in range(0, len(texts_to_embed), batch_size):
            ...
            report_progress(
                3,
                f"Vérification profonde... ({len(all_full_embeddings)}/{len(texts_to_embed)})",
                ratio,
            )
...
"""
    ...
    def process_fusion_group(group):
        ...
        with progress_lock:
            ...
            report_progress(
                4,
                f"Fusion des doublons : groupe {completed_groups}/{len(duplicate_groups)}",
                ratio,
            )
        print(f"      ────────────────────────────────────────")
...
def process_course(g_file, learning_depth="Zero Lecture"):
    ...
    try:
        ...
        print(
            f"--> Environnement isolé : tous les fichiers (logs, images, deck) seront dans `{abs_run_folder}`\n"
        )
    except Exception as e:
    ...
    try:
        ...
        def process_single_chunk(idx, chunk):
            ...
            if not chunk.strip():
                with progress_lock:
                    ...
                    report_progress(
                        2, f"Création : bloc {completed_chunks}/{len(chunks)}", ratio
                    )
                return idx, []
            print(
            ...
            cards = extract_cards_from_chunk(
                client, chunk, learning_depth, filename_tag=filename_without_ext
            )
            if cards is None:
                with progress_lock:
                    completed_chunks += 1
            ...
            def retry_single_chunk(idx, chunk):
                ...
                if cards is None:
                    ...
                    with progress_lock:
                        ...
                        report_progress(
                            2,
                            f"Création : bloc {completed_chunks}/{len(chunks)}",
                            ratio,
                        )
                    return idx, []
                cards = ai_quality_control_cards(
        ...
        if learning_depth == "Zero Lecture":
            all_cards = supervisor_deduplicate_cards(
                client, all_cards, 0.88, image_descriptions=image_descriptions
            )
        else:
            all_cards = supervisor_deduplicate_cards(
                client, all_cards, 0.86, image_descriptions=image_descriptions
            )
        ...
        if learning_depth == "Zero Lecture":
            ...
            my_package = genanki.Package(
                [deck_definitions, deck_theoremes, deck_vocabulaire]
            )
